...
l.24: もしfpがNULLだったときは，ファイルを開けなかったので即座にプログラム終了

l.29: countに1を代入

l.30: ファイルを1行ずつbufに読み込む．もし読み込み成功する限り以下を繰り返す

l.31: bufからsscanfで数値文字列を実数としてvalに読み込む
...
## 修正履歴
[comment #20200618]
- レポートの最初の部分は自分の学籍番号, 氏名にしてください. 
- 出力結果を貼り付けてください
- 「標本平均」「標本分散」「母集団平均(推定値)」「母集団分散(推定値)」の4つを出力してください. 
- 2乗したものの平均を求める関数(square_ave_online())を作っていますが, ave_online()を利用することで, 2乗したものの平均を求めることができます.   
呼び出すときの引数を工夫して, `square_ave_online()を使わず, ave_online()を使って`2乗したものの平均を求めましょう. 
- l.34に不要な `=` があります.   
